...
use railq::{
    ...
    model::{
        ...
        RailStationId, SpeedMetresPerSecond, UtcSeconds,
    },
    sim::{
...
fn finance_fixture(waiting_passengers: u32) -> GameState {
    ...
    state.rules.balance = BalanceConfig::new(
        ...
        vec![DieselTrainCatalogueRecord::new(
            ...
            PassengerCapacity::new(4).expect("fixture capacity is positive"),
            SpeedMetresPerSecond::new(100).expect("fixture speed is positive"),
            fuel_rate,
        )],
    );
    state.player_company.funds = Money::from_cents(2_000);
...
-> Result<(), Box<dyn Error>> {
    ...
    for (slug, state, funds, revenue, result, status) in fixtures {
        ...
        for (columns, rows, size) in [(120, 40, "120x40"), (80, 24, "80x24")] {
            ...
            assert!(rendered.contains("$0.50"), "{slug} should show fuel amount");
            assert!(
